...
contract SwanToken is ERC20("SwanToken", "SWAN"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (SwanMaster).
...
contract SwanMaster is Ownable {
    ...
    // Block number when bonus SWAN period ends.
    ...
    function changeSwanPerBlock(uint256 _newSwanPerBlock) public onlyOwner {
        swanPerBlock = _newSwanPerBlock;
    }
    ...
    // Withdraw LP tokens from SwanMaster.
